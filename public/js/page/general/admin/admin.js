var url = $('meta[name="__global_url"]').attr('content')+'/admin-master'
            ...
            if (res.code === 200) {
                ...
                $(res.data.data).each(function (index, item) {
                    $('#tableData tbody').append(
                        '<tr>'
                            +'<td><input type="checkbox" name="check" onclick="selectData('+item.admin_id+')" value="'+ item.admin_id +'" class="checkbox-template"></td>'
                            +'<td>' + (parseInt, res.data.from+index) + '</td>'
                            +'<td>' + item.admin_name + '</td>'
                            +'<td>' + item.admin_title + '</td>'
                            +'<td>' + item.role_name + '</td>'
                            +'<td>'
                                +'<button type="button" name="button" class="btn btn-warning btn-sm" onclick="detail('+item.admin_id+')">Detail</button>'
                                +'&nbsp'
                                +'<button type="button" name="button" class="btn btn-success btn-sm" onclick="edit('+item.admin_id+')">Edit</button>'
                                +'&nbsp'
                                +'<button type="button" name="button" class="btn btn-danger btn-sm" onclick="remove('+item.admin_id+')">Delete</button>'
                            +'</td>'
                        +'</tr>'
                    )
                })
