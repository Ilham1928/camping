var url = $('meta[name="__global_url"]').attr('content')+'/menu/child'
            ...
            if (res.code === 200) {
                ...
                $(res.data.data).each(function (index, item) {
                    $('#tableData tbody').append(
                        '<tr>'
                            +'<td><input type="checkbox" name="check" onclick="selectData('+item.menu_child_id+')" value="'+ item.menu_child_id +'" class="checkbox-template"></td>'
                            +'<td>' + (parseInt, index+1) + '</td>'
                            +'<td>' + item.menu_child_name + '</td>'
                            +'<td>' + item.menu_child_url + '</td>'
                            +'<td>' + item.menu_parent_name + '</td>'
                            +'<td>'
                                +'<button type="button" name="button" class="btn btn-success btn-sm" onclick="edit('+item.menu_child_id+')">Edit</button>'
                                +'&nbsp'
                                +'<button type="button" name="button" class="btn btn-danger btn-sm" onclick="remove('+item.menu_child_id+')">Delete</button>'
                            +'</td>'
                        +'</tr>'
                    )
                })
...
function save() {
    var params = {
            'menu_child_name' : $('input[name=menu_child_name]').val(),
            'menu_child_url'  : $('input[name=menu_child_url]').val(),
            'menu_parent_id'  : $('select[name=menu_parent_id] :selected').val()
        }
